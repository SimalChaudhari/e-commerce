...
export class LedgerService {
  ...
  async fetchAndStoreLedgers(): Promise<void> {
    ...
    try {
      ...
      // Iterate through the fetched outstanding data and compare
      for (const outstanding of outstandingData) {
        ...
        if (existingOutstanding) {
          ...
          if (this.hasStatementChanges(existingOutstanding, outstanding)) {
            ...
            await this.ledgerRepository.save({
              ...existingOutstanding,
              ...outstanding,
              bills: await this.processBills(
                existingOutstanding, outstanding.bills),
            });


          } else {
            console.log(`No changes for outstanding customer: ${outstanding.customerName}`);
          }
        } else {
          const saved = await this.ledgerRepository.save(outstanding);

          // Save vouchers and associate them with the new ledger
          if (outstanding.bills) {
            for (const bills of outstanding.bills) {
              bills.ledger = saved;
              await this.saveIfNotDuplicate(bills);
            }
          }
        }
      }
      ...
      for (const ledger of ledgerData) {
        ...
        if (existingLedger) {
          ...
          if (this.hasLedgerStatementChanges(existingLedger, ledger)) {
            await this.ledgerStatementRepo.save({
              ...existingLedger,
              ...ledger,
              vouchers: await this.processVouchers(
                existingLedger, ledger.vouchers),
            });
          } else {
            console.log(`No changes for ledger party: ${ledger.party}`);
          }
        } else {
          ...
          if (ledger.vouchers) {
            for (const voucher of ledger.vouchers) {
              ...
              await this.saveVoucherIfNotDuplicate(voucher);
            }
          }
        }
      }
