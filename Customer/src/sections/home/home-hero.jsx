...
export function HomeHero({ sx, ...other }) {
  ...
  const scroll = useScrollPercent(); // Get scroll percentage

  const mdUp = useResponsive('up', mdKey); // Detect medium screen size and up
  const distance = mdUp ? scroll.percent : 0; // Set distance based on scroll percent and screen size

  // Define scroll-based animations for various elements
  ...
  // Control the opacity based on scroll
...
// Utility to handle Y-axis transformation during scroll
...
// Custom hook to calculate scroll percentage
